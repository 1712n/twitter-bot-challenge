...
class MarketCapBot:
    ...
    def __init__(self, ohlcv_db: pymongo.collection.Collection,
    ...
    def get_pair_to_post(self) -> str:
        ...
        """
        top_pairs = db.get_top_pairs(self.ohlcv_db, max_num=100)
    ...
    def _get_message_dict(self,
        ...
        if pair is None and (pair_symbol is None or pair_base is None):
            raise ValueError(
                "Either pair or pair_symbol and pair_base must be provided")
        elif pair is None:
            ...
            try:
                ...
                raise ValueError(
                    "Invalid pair provided, must be in the format 'SYMBOL-BASE"
                )
            ...
            if item['timestamp'] == latest_date:
                if item['marketVenue'] not in message_dict:
                    if len(message_dict) <= 5:
                        ...
                        message_dict[item['marketVenue']] = float(
                            item['volume'])
                    else:
        ...
        message_dict = {
            key: round((value / total_volume) * 100, 2)
    ...
    def compose_message(self,
        ...
        other_percentage = message_dict.pop('others')
        ...
        if other_percentage != 0:
            message += f"Others: {other_percentage}%\n"
        logging.info("Message composed!")
        return message

    @db.mongodb_write_error_handler
    ...
    def post_message(self, pair: str = None, message: str = None) -> None:
        ...
        pair_main_post = self.posts_db.find({
            'pair': pair,
            'tweet_id': {
                '$exists': True
            }
        }).sort('time', pymongo.ASCENDING).limit(1)
        tweet_id = None
        try:
            ...
            logging.warning(
                "A tweet with the same text has already been posted to this thread"
            )
            ...
            self._save_message_to_db(pair=pair,
                                     tweet_id=tweet_id,
                                     message=message)
            return
...
if __name__ == "__main__":
    ...
    ohlcv_db = db.get_mongo_collection('ohlcv_db')
    posts_db = db.get_mongo_collection('posts_db')
